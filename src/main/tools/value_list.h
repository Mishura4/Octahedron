...
struct value_list_helper {
	...
	template <size_t N> requires (N < sizeof...(Args))
...
struct value_duplicates_helper_s {
	...
	static consteval auto is_duplicate_fun() {
		using T = decltype(X);
		using U = decltype(Y);
		constexpr bool is_comparable = (exact_type
																			? std::is_same_v<T, U>
...
struct value_list_without_duplicates_s {
	using type = std::conditional_t<is_duplicate_value<true, T, Args...>,
																	...
																	typename value_list_cat_s<
																		value_list<T>, typename value_list_without_duplicates_s<Args
																			...>::type>::type
	>;
};
...
struct value_list_cat_s<lhs, rhs> {
	using type = decltype(value_list_cat_helper<
		lhs, rhs, std::make_index_sequence<lhs::size + rhs::size>>::fun());
};
