...
namespace Octahedron
{
  ...
  namespace _
  {
    ...
    struct tuple_without_duplicates_s
    {
      using type = std::conditional_t <is_duplicate_type<T, Args...>,
        ...
        decltype(std::tuple_cat(std::declval<std::tuple<T>>(), std::declval<typename tuple_without_duplicates_s<Args...>::type>()))
      >;
    };
  ...
  protected:
    ...
    static consteval size_t type_index_fun()
    {
      ...
      if constexpr (std::is_same_v<T, U>)
        return (0);
      else if constexpr (constexpr auto pos = type_list<Args...>::template type_index_fun<U>(); pos == npos)
        return (npos);
      else // constexpr
        return (1 + pos);
    }
  ...
  public:
    ...
    using without_duplicate_types = typename _::type_list_duplicate_helper<has_duplicate_types, T, Args...>::type;
