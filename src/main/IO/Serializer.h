...
{
	template <typename T>
	requires(std::ranges::range<T> && byteoid<std::remove_cvref<std::ranges::range_value_t<T>>>)
	class Serializer : public
			std::conditional_t<
				std::is_const_v<std::remove_cvref<std::ranges::range_value_t<T>>>,
				IOReadable<Serializer<T>>,
				IOInterface<Serializer<T>>>
	{
		public:
		  using value_type = std::remove_reference_t<std::ranges::range_value_t<T>>;
			...
			Serializer(value_type *data, size_t size)
			  requires(std::constructible_from<T, value_type *, size_t>) :
				_data{data, size}
			{

			}

			Serializer(std::initializer_list<value_type> values)
			requires(std::constructible_from<T, std::initializer_list<value_type>>)
					: _data(values), _pos(std::end(_data))
			{}
			...
			Serializer &operator=(std::initializer_list<value_type> values)
			requires(std::assignable_from<T &, std::initializer_list<value_type>>)
			{
				_data = values;
				_pos = 0;
				return (*this);
			...
			size_t read(std::span<std::byte> buf)
			{
				...
				size_t to_read = min(buf.size(), bytesAvailable());
			...
			size_t write(std::span<const std::byte> buf)
			{
				...
				{
					size_t to_write = min(bytesAvailable(), buf.size());
			...
			requires(requires(T &t) { t.reserve(size); })
			{
				_data.reserve(size);
			}
			...
			auto build() const -> std::vector<value_type>
			{
	...
	using DynamicBuffer = Serializer<std::vector<std::byte>>;

	template <byteoid T, size_t Size>
	using StaticBuffer = Serializer<std::array<T, Size>>;
